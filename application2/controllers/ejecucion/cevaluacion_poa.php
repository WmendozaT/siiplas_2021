  ...
  public function __construct (){
    ...
    public function formulario_evaluacionpoa($com_id){
      ...
      if(count($data['componente'])!=0){
          ...
          $titulo=
          '<h1 title='.$data['proyecto'][0]['aper_id'].'><small>PROGRAMA : </small>'.$data['proyecto'][0]['aper_programa'].''.$data['proyecto'][0]['aper_proyecto'].''.$data['proyecto'][0]['aper_actividad'].' - '.$data['proyecto'][0]['tipo'].' '.$data['proyecto'][0]['proy_nombre'].' - '.$data['proyecto'][0]['abrev'].'</h1>
          <h1><small>UNIDAD RESPONSABLE : </small> '.$data['componente'][0]['tipo_subactividad'].' '.$data['componente'][0]['serv_cod'].' '.$data['componente'][0]['serv_descripcion'].'</h1>';
...
/*                          $programado=$this->model_evaluacion->programado_trimestral_productos($this->tmes,$rowp['prod_id']); //7 Meta programado Trimestre
    ...
    public function obtiene_datos_evaluacíon($com_id,$trimestre,$tipo_evaluacion){
      ...
      //for ($i=1; $i <=$trimestre; $i++) {
        $programadas=$this->model_evaluacion->nro_operaciones_programadas($com_id,$trimestre); //// Nro de Operaciones
        $suma_programado=$this->model_evaluacion->suma_operaciones_programadas($com_id,$trimestre); /// suma meta trimestral
        ...
        if(count($this->model_evaluacion->list_operaciones_evaluadas_servicio_trimestre_tipo($com_id,$trimestre,$tipo_evaluacion))!=0){
          $nro_cumplidas=count($this->model_evaluacion->list_operaciones_evaluadas_servicio_trimestre_tipo($com_id,$trimestre,$tipo_evaluacion));
        }
      
        $suma_evaluado=$this->model_evaluacion->suma_operaciones_ejecutadas($com_id,$trimestre);
    ...
    public function tabla_acumulada_evaluacion_servicio($regresion,$tp_graf,$tip_rep){
      $tabla='';
      $tit[2]='TOTAL ACT. PROG.';
      $tit[3]='TOTAL ACT. CUMP.';
      $tit[4]='ACT. NO CUMP';
      $tit[5]='% ACT. CUMP.';
      $tit[6]='% ACT. NO CUMP.';

      $tit_total[2]='TOTAL ACT. PROG.';
      $tit_total[3]='TOTAL ACT. CUMP.';
      $tit_total[4]='% ACT. PROG.';
      $tit_total[5]='% ACT. CUMP.';
    ...
    function update_evaluacion(){
      if ($this->input->post()) {
          ...
          if(count($this->model_evaluacion->verif_com_eval($com_id,$this->tmes))==1){
            $this->session->set_flashdata('success','SE REALIZO LA ACTUALIZACIÓN DE FORM 4 NO CUMPLIDAS');
          }
          else{
    ...
    public function mi_poa($proy_id){
      ...
      $tabla.=' <table class="table table-bordered">
                  ...
                  <tr>
                    ...
                    <th style="width:80%;" bgcolor="#474544"> UNIDAD RESPONSABLE</th>
