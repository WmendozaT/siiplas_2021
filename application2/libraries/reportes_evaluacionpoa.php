...
class reportes_evaluacionpoa extends CI_Controller{
  ...
  public function pie_evaluacionpoa(){
    ...
    public function obtiene_datos_evaluacíon_programa_institucional($aper_programa,$tipo_evaluacion){
      $nro_ope_eval=0; $nro_cumplidas=0;
        $programadas=$this->model_evalprograma->nro_form4_programadas_institucional($aper_programa);
        if(count($programadas)!=0){
          $nro_ope_eval=$programadas[0]['total'];
        }

        if(count($this->model_evalprograma->list_operaciones_evaluadas_institucional_trimestre($aper_programa,$this->tmes,$tipo_evaluacion,4))!=0){
          $nro_cumplidas=count($this->model_evalprograma->list_operaciones_evaluadas_institucional_trimestre($aper_programa,$this->tmes,$tipo_evaluacion,4));
        }
    ...
    /*--- OBTIENE DATOS DE EVALUACIÓN 2024 - APERTURA PROGRAMATICA REGIONAL ---*/
    ...
    /*--- OBTIENE DATOS DE EVALUACIÓN 2020 - APERTURA PROGRAMATICA DISTRITAL ---*/
