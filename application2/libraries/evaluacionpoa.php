...
////================== PARAMETROS DE EFICACIA 
  ...
  /*---- matriz parametros de eficacia Distrito ----*/
    ...
    public function obtiene_datos_evaluacíon_por_regional($dep_id,$trimestre,$tipo_evaluacion){
      ...
      $programadas=$this->model_evalinstitucional->nro_operaciones_programadas_regional($dep_id,$trimestre,4);
      if(count($programadas)!=0){
        $nro_ope_eval=$nro_ope_eval+$programadas[0]['total'];
      }

      if(count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_regional($dep_id,$trimestre,$tipo_evaluacion,4))!=0){
        $nro_cumplidas=$nro_cumplidas+count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_regional($dep_id,$trimestre,$tipo_evaluacion,4));
      }
    ...
    public function obtiene_datos_evaluacíon_por_unidad($proy_id,$trimestre,$tipo_evaluacion){
        $nro_ope_eval=0; $nro_cumplidas=0;
        $programadas=$this->model_evalunidad->nro_operaciones_programadas($proy_id,$trimestre);
        if(count($programadas)!=0){
          $nro_ope_eval=$programadas[0]['total'];
        }

        if(count($this->model_evalunidad->list_operaciones_evaluadas_unidad_trimestre_tipo($proy_id,$trimestre,$tipo_evaluacion))!=0){
          $nro_cumplidas=count($this->model_evalunidad->list_operaciones_evaluadas_unidad_trimestre_tipo($proy_id,$trimestre,$tipo_evaluacion));
        }
...
/*    public function eficiencia_unidad($eficacia,$economia){
 ...
 /*===== CONSOLIDADO NACIONAL =====*/
    ...
    public function tabla_regresion_lineal_nacional_total(){
      ...
      $programado=$this->model_evalinstitucional->nro_operaciones_programadas_nacional(4,4);
        if(count($programado)!=0){
          $total=$programado[0]['total'];
        }
    ...
    public function obtiene_datos_evaluacíon_nacional($trimestre,$tipo_evaluacion){
      ...
      /*for ($i=1; $i <=$trimestre; $i++) {
        ...
        $programadas=$this->model_evalinstitucional->nro_operaciones_programadas_nacional($trimestre,4);
        if(count($programadas)!=0){
          $nro_ope_eval=$programadas[0]['total'];
        }

        if(count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_nacional($trimestre,$tipo_evaluacion,4))!=0){
          $nro_cumplidas=count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_nacional($trimestre,$tipo_evaluacion,4));
        }
  ...
  /*================*/
    ...
    public function tabla_regresion_lineal_regional_total($dep_id){
      ...
      $programado=$this->model_evalinstitucional->nro_operaciones_programadas_regional($dep_id,4,4);
        if(count($programado)!=0){
          $total=$programado[0]['total'];
        }
    ...
    /*--- OBTIENE DATOS DE EVALUACIÓN - REGIONAL ---*/
    public function obtiene_datos_evaluacíon_regional($dep_id,$trimestre,$tipo_evaluacion){
      ...
      /*for ($i=1; $i <=$trimestre; $i++) {
        ...
        if(count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_regional($dep_id,$trimestre,$tipo_evaluacion,4))!=0){
          $nro_cumplidas=count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_regional($dep_id,$trimestre,$tipo_evaluacion,4));
        }
  ...
  /*================*/
    ...
    public function tabla_regresion_lineal_distrital_total($dist_id){
      ...
      $programado=$this->model_evalinstitucional->nro_operaciones_programadas_distrital($dist_id,4,4); ///total programado
      if(count($programado)!=0){
        $total=$programado[0]['total'];
      }
    ...
    public function obtiene_datos_evaluacíon_distrital($dist_id,$trimestre,$tipo_evaluacion){
      ...
      //for ($i=1; $i <=$trimestre; $i++) {
        $programadas=$this->model_evalinstitucional->nro_operaciones_programadas_distrital($dist_id,$trimestre,4);
        $suma_programado=$this->model_evalinstitucional->suma_operaciones_programadas_distrital($dist_id,$trimestre); /// suma meta trimestral
        ...
        if(count($suma_programado)!=0){
          $total_programado=$suma_programado[0]['suma_programado'];
        }

        if(count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_distrital($dist_id,$trimestre,$tipo_evaluacion,4))!=0){
          $nro_cumplidas=$nro_cumplidas+count($this->model_evalinstitucional->list_operaciones_evaluadas_unidad_trimestre_tipo_distrital($dist_id,$trimestre,$tipo_evaluacion,4));
        }

        $suma_evaluado=$this->model_evalinstitucional->suma_operaciones_ejecutadas_distrital($dist_id,$trimestre);

        if(count($suma_evaluado)!=0){
          $total_ejecutado=$suma_evaluado[0]['suma_evaluado'];
        }
     // }
