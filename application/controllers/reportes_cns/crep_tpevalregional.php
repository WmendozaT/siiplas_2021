...
class Crep_tpevalregional extends CI_Controller {  
    ...
    public function __construct (){
        ...
        if($this->session->userdata('fun_id')!=null){
            ...
            $this->load->model('reporte_eval/model_tpevalregional');
    ...
    public function consolidado_tpregional($dep_id,$tp_id){
      $data['menu']=$this->menu(7); //// genera menu
      $data['dep']=$this->model_tpevalregional->get_dpto($dep_id);
      if(count($data['dep'])!=0){
        $data['dist']=$this->model_tpevalregional->get_distrital($dep_id);
        ...
        $data['tipo']='PROYECTO DE INVERSIÓN';
        if($tp_id==4){
          $data['tipo']='GASTO CORRIENTE';
        }

        $data['tabla']=$this->proyectos_regional($dep_id,$tp_id);
        $data['print_regional']=$this->print_proyectos_distrital($dep_id,$data['tabla'],$tp_id); /// print Eficacia Regional

        /*--------- Cuadro Evaluacion de operaciones por Programas -----------------*/
        $data['eval_trimestral']=$this->cuadro_comparativo_programas_evaluado($dep_id,$tp_id)[0];
        $data['evaluacion']=$this->evaluacion_operaciones_regional($dep_id,$tp_id);
        $data['imprimir_evaluacion']=$this->print_evaluacion_operaciones_regional($data['dep'],$tp_id);

        /*--------- Grafico (torta) parametros de eficacia -----------------*/
        $data['nro']=$this->nro_list_distrital($dep_id,$tp_id);
        $data['eficacia']=$this->eficacia_distrital($data['nro'],$dep_id,1,$tp_id); //// Grafico de Parametros de eficacia
        $data['print_eficacia']='Trabajando';


        $this->load->view('admin/reportes_cns/eval_regional/reporte_eval/eval_tpconsolidado_regional', $data);
      }
      else{
        redirect('admin/dashboard');
      }
    ...
    public function print_evaluacion_operaciones_regional($dep,$tp_id){
      $tr=($this->tmes*3);
      $mes = $this->mes_nombre();
      $trimestre=$this->model_evaluacion->trimestre();

      $tipo='PROYECTO DE INVERSIÓN';
      if($tp_id==4){
        $tipo='GASTO CORRIENTE';
      }

      $eval=$this->cuadro_comparativo_programas_evaluado($dep[0]['dep_id'],$tp_id)[1];
      ...
      $tabla .='<table width="90%" align=center border=0>
                ...
                <hr>';
        $tabla .='<center><FONT FACE="courier new" size="2"><b>CUADRO DE EVALUACI&Oacute;N ACUMULADA DE OPERACIONES AL '.$trimestre[0]['trm_descripcion'].'<br>'.$tipo.'<b/></FONT></center>';
        $tabla.=''.$this->print_evaluacion_acu_operaciones($dep[0]['dep_id'],$tp_id).'<br>
    ...
    public function print_evaluacion_acu_operaciones($dep_id,$tp_id){
      $programas=$this->model_tpevalregional->categorias_programaticas_regional($dep_id,$tp_id);
      $tabla='';
    ...
    public function evaluacion_operaciones_regional($dep_id,$tp_id){
      $dep=$this->model_tpevalregional->get_dpto($dep_id);
      $eval=$this->cuadro_comparativo_programas_evaluado($dep_id,$tp_id)[1];
      $tmes=$this->model_evaluacion->trimestre();
    ...
    public function proyectos_regional($dep_id,$tp_id){
      $m[1]='Ene.';
      $m[2]='Feb.';
      $m[3]='Mar.';
      $m[4]='Abr.';
      ...
      $proyectos=$this->model_tpevalregional->list_consolidado_regional($dep_id,$tp_id);
      ...
      $cum=$this->model_tpevalregional->evaluacion_productos_regional($dep_id,1,$tp_id); // Cumplido
      $pro=$this->model_tpevalregional->evaluacion_productos_regional($dep_id,2,$tp_id); // En Proceso
      $ncum=$this->model_tpevalregional->evaluacion_productos_regional($dep_id,3,$tp_id); // No Cumplido
    ...
    public function print_proyectos_distrital($dep_id,$p,$tp_id){
      $dep=$this->model_tpevalregional->get_dpto($dep_id);
      $tr=($this->tmes*3);
      $mes = $this->mes_nombre();
      $trimestre=$this->model_evaluacion->trimestre();
      ...
      $tabla .='<table width="90%" align=center border=0>
                ...
                <hr>';
        $tabla .='<center><FONT FACE="courier new" size="2"><b>CUADRO DE EJECUCI&Oacute;N AL '.$trimestre[0]['trm_descripcion'].'<b/><br>'.$tipo.'</FONT></center>';
        $tabla .='<table class="change_order_items" border=1 style="width:100%;">
                  <tr>
                    <td>
                      <div id="regresion2" style="width: 690px; height: 295px; margin: 0 auto"></div>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <div id="container2" style="width: 690px; height: 295px; margin: 0 auto"></div>
                    </td>
                  </tr>
                  <tr>
                    <td colspan=2>
    ...
    public function cuadro_comparativo_programas_evaluado($dep_id,$tp_id){
      $tabla ='';
      $programas=$this->model_tpevalregional->categorias_programaticas_regional($dep_id,$tp_id);
    ...
    public function eficacia_distrital($matriz,$dep_id,$tp,$tp_id){
        if($tp==1){
    ...
    public function nro_list_distrital($dep_id,$tp_id){
    //  $dist=$this->model_tpevalregional->get_dist($dist_id);
      $acciones=$this->model_tpevalregional->list_consolidado_regional($dep_id,$tp_id);
    ...
    public function matriz_evaluacion_trimestre($dep_id,$aper_programa){
      ...
      $nro_cum=0;$nro_proc=0;$nro_ncum=0;$nro_total_prog=0; $nro_cum_a=0;$nro_proc_a=0;$nro_ncum_a=0;$nro_total_prog_a=0;
        ...
        $cum=$this->model_tpevalregional->evaluacion_programas_regional($dep_id,$aper_programa,1,$this->tmes);
        $proc=$this->model_tpevalregional->evaluacion_programas_regional($dep_id,$aper_programa,2,$this->tmes);
        $ncum=$this->model_tpevalregional->evaluacion_programas_regional($dep_id,$aper_programa,3,$this->tmes);
        $total_prog=$this->model_tpevalregional->total_programado_programas_regional($dep_id,$aper_programa,$this->tmes); // total programado prod
    ...
    public function matriz_evaluacion_Acumulado($dep_id,$aper_programa){
      ...
      for ($i=1; $i <=$this->tmes ; $i++) { 
        ...
        $cum=$this->model_tpevalregional->evaluacion_programas_regional($dep_id,$aper_programa,1,$i);
        $proc=$this->model_tpevalregional->evaluacion_programas_regional($dep_id,$aper_programa,2,$i);
        $ncum=$this->model_tpevalregional->evaluacion_programas_regional($dep_id,$aper_programa,3,$i);
        $total_prog=$this->model_tpevalregional->total_programado_programas_regional($dep_id,$aper_programa,$i); // total programado prod
    ...
    public function list_distritales($dep_id){
      $tmes=$this->model_evaluacion->trimestre();
      $dist=$this->model_tpevalregional->get_distrital($dep_id);
      $tabla ='';
    ...
    public function proyectos_distrital($dep_id,$dist_id){
      ...
      $proyectos=$this->model_tpevalregional->list_consolidado_distrital($dep_id,$dist_id);
      foreach($proyectos  as $rowp){
        $tabla=$this->componentes($rowp['proy_id']);
        for ($i=1; $i <=12 ; $i++) { 
          $p[1][$i]=$p[1][$i]+round((($tabla[1][$i]*$rowp['proy_pcion_reg'])/100),2);
      ...
      $cum=$this->model_tpevalregional->evaluacion_distrital_acumulado($dist_id,1); // Cumplido
      $pro=$this->model_tpevalregional->evaluacion_distrital_acumulado($dist_id,2); // En Proceso
      $ncum=$this->model_tpevalregional->evaluacion_distrital_acumulado($dist_id,3); // No Cumplido
      $total_prog=$this->model_tpevalregional->total_programado_distrital_acumulado($dist_id); // total programado
      $nro_1=0;$nro_2=0;$nro_3=0;
      if(count($cum)!=0){
        $nro_1=$cum[0]['total'];
      }
