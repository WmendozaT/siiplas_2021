<?php
class Crep_evalofinacional extends CI_Controller {  
    ...
    public function get_cuadro_evaluacion_onacional(){
      if($this->input->is_ajax_request() && $this->input->post()){
        ...
        $id = $this->security->xss_clean($post['id']); // id unidad
        $tp = $this->security->xss_clean($post['tp']); // 1 : Gerencia General, 2: Administrativa, 3 : Gerencia Salud

        $tabla='<iframe id="ipdf" width="100%" height="1000px;" src="'.base_url().'index.php/rep_eval_poa/evaluacion_poa_onacional/'.$id.'"></iframe>';
    ...
    public function evaluacion_poa_onacional($proy_id){
      $data['trimestre']=$this->model_evaluacion->trimestre(); /// Datos del Trimestre
      $data['boton']='MOSTRAR CUADRO DE UNIDADES';
      $data['proyecto'] = $this->model_proyecto->get_datos_proyecto_unidad($proy_id);
      if(count($data['proyecto'])!=0){
        $data['titulo']=
        '<h1><b>EVALUACI&Oacute;N POA - CONSOLIDADO '.$data['proyecto'][0]['aper_programa'].''.$data['proyecto'][0]['aper_proyecto'].''.$data['proyecto'][0]['aper_actividad'].' - '.$data['proyecto'][0]['tipo'].' '.$data['proyecto'][0]['proy_nombre'].' - '.$data['proyecto'][0]['abrev'].'</b></h1>
        ...
        $data['calificacion']=$this->calificacion_eficacia($data['tabla'][5][$this->tmes]); /// Parametros de Eficacia
                    ...
                    <table '.$class.'>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td>0% a 75%</td>
                          <td><a class="btn btn-danger" style="width: 100%" align="left" title="'.$matriz[1][2].' Unidades/Proyectos">'.$matriz[1][2].'</a></td>
                        </tr>
                        <tr>
                          <td>REGULAR</td>
                          <td>75% a 90% </td>
                          <td><a class="btn btn-warning" style="width: 100%" align="left" title="'.$matriz[2][2].' Unidades/Proyectos">'.$matriz[2][2].'</a></td>
                        </tr>
                        <tr>
                          <td>BUENO</td>
                          <td>90% a 99%</td>
                          <td><a class="btn btn-info" style="width: 100%" align="left" title="'.$matriz[3][2].' Unidades/Proyectos">'.$matriz[3][2].'</a></td>
                        </tr>
                        <tr>
                          <td>OPTIMO </td>
                          <td>100%</td>
                          <td><a class="btn btn-success" style="width: 100%" align="left" title="'.$matriz[4][2].' Unidades/Proyectos">'.$matriz[4][2].'</a></td>
                        </tr>
                        <tr>
 ...
 /*------ Parametro de eficacia ------*/
    ...
    public function print_proyectos_unidad($proy_id,$regresion,$regresion_total,$mis_servicios,$economia,$eficacia,$eficiencia,$calificacion,$parametro){
      ...
      $tabla .='<div class="verde"></div>
        ...
        $tabla .='<table class="change_order_items" border=0 style="width:100%;">
        ...
        $tabla .='<div style="font-size: 6.5pt;"><b>(%) EFICACIA - Nivel de Cumplimiento de Actividades</b></div><br>
                  <div style="font-size: 40pt;" align=center><b>'.$eficacia.' % </b></div><br>

                  <div style="font-size: 6.5pt;"><b>(%) ECONOMIA - Presupuesto Ejecutado</b></div><br>
                  <div style="font-size: 40pt;" align=center><b>'.$economia[3].' % </b></div><br>
                  <div style="font-size: 5pt;"><b>PRESUPUESTO ASIGNADO : </b> '.number_format($economia[2], 2, ',', '.').' Bs. - <b>PRESUPUESTO EJECUTADO : </b> '.number_format($economia[1], 2, ',', '.').' Bs.</div><br>

                  <div style="font-size: 6.5pt;"><b>(%) EFICIENCIA - (Eficacia/Economia)</b></div><br>
                  <div style="font-size: 40pt;" align=center><b>'.$eficiencia.' </b></div>
 ...
 /*------ TABLA ACUMULADA EVALUACIÓN 2020 -------*/
    public function tabla_acumulada_evaluacion_unidad($regresion,$tp_graf,$tip_rep){
      $tabla='';
      $tit[2]='TOTAL PROGRAMADO';
      $tit[3]='TOTAL CUMPLIDOS';
      $tit[4]='NO CUMPLIDOS';
      $tit[5]='% CUMPLIDOS';
      $tit[6]='% NO CUMPLIDOS';

      $tit_total[2]='TOTAL PROGRAMADO';
      $tit_total[3]='TOTAL CUMPLIDOS';
      $tit_total[4]='% PROGRAMADO';
      $tit_total[5]='% CUMPLIDO';
    ...
    public function tabla_regresion_lineal_unidad_total($proy_id){
      ...
      for ($i=1; $i <=4; $i++) {
        $valor=$valor=$this->obtiene_datos_evaluacíon($proy_id,$i,1);
        $tr[2][$i]=$valor[1]; /// Prog
        $tr[3][$i]=$valor[2]; /// cumplidas
        if($total!=0){
