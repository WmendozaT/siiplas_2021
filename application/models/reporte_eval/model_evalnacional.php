<?php
class Model_evalnacional extends CI_Model{
    var $gestion;
    ...
    public function evaluacion_institucional_regional($dep_id,$teval,$trimestre){
        $sql = 'select p.dep_id,ev.tp_eval,SUM(ev.nro) as total
                ...
                where p.estado!=\'3\' and apg.aper_gestion='.$this->gestion.' and apg.aper_estado!=\'3\' and (apg.aper_programa!=\'97\' and apg.aper_programa!=\'98\') and apg.aper_proy_estado=\'4\' and pf.pfec_estado=\'1\'
                and p.dep_id='.$dep_id.' and ev.g_id='.$this->gestion.' and ev.trm_id='.$trimestre.' and ev.tp_eval='.$teval.' and p.dist_id!=\'0\'
    ...
    public function total_institucional_regional($dep_id,$trimestre){
        ...
        $sql = 'select p.dep_id,count(pprog.prod_id) total
               from _proyectos as p
               Inner Join _tipoproyecto as tp On p.tp_id=tp.tp_id
               Inner Join aperturaproyectos as ap On ap.proy_id=p.proy_id
               Inner Join aperturaprogramatica as apg On apg.aper_id=ap.aper_id
               ...
               and p.dep_id='.$dep_id.' and (apg.aper_programa!=\'97\' and apg.aper_programa!=\'98\') and p.dist_id!=\'0\' and vprod.estado!=\'3\'
               GROUP BY p.dep_id';
        $query = $this->db->query($sql);
        return $query->result_array();
    }
