...
class Model_evalprograma extends CI_Model{
    ...
    public function lista_apertura_programas_institucional($tp_id){
            if($this->gestion>2022){
                ...
                Inner Join _componentes as c On c.pfec_id=poa.pfec_id
                Inner Join _productos as prod On prod.com_id=c.com_id
                where c.estado!=\'3\' and prod.estado!=\'3\' and apg.aper_asignado=\'1\'
                group by apg.aper_programa,apg.aper_proyecto,apg.aper_actividad,apg.aper_descripcion
                order by apg.aper_programa asc';
            }
    ...
    /*----- NUMERO DE FORM 4 PROGRAMADAS POR TRIMESTRE / APERTURA INSTITUCIONAL 2024------*/
    public function nro_form4_programadas_institucional($programa){
        $sql = '
                ...
                Inner Join _productos as pr On pr.com_id=c.com_id
                 Inner Join (
                    ...
                    where (m_id>\'0\' and m_id<='.($this->tmes*3).') and pg_fis!=\'0\'
                    group by prod_id
                ) as pprog On pprog.prod_id=pr.prod_id
                
                where poa.prog=\''.$programa.'\' and c.estado!=\'3\' and pr.estado!=\'3\'
                group by poa.prog';
    ...
    /*----- NUMERO DE OPERACIONES PROGRAMADAS POR TRIMESTRE / APERTURA REGIONAL 2020 -2121-2122-2023------*/
/*    public function nro_operaciones_programadas_institucional($programa,$trimestre,$tp_id){
        ...
        if($this->gestion>2022){
            ...
            where apg.aper_programa=\''.$programa.'\' and c.estado!=\'3\' and prod.estado!=\'3\' and apg.aper_asignado=\'1\'
            group by apg.aper_programa';
        }
    ...
    public function list_operaciones_evaluadas_institucional_trimestre($programa,$trimestre,$tipo_eval,$tp_id){
        if($this->gestion>2022){
            ...
            $sql = 'select poa.prog aper_programa,poa.proy aper_proyecto,poa.act aper_actividad,poa.proy_id,c.com_id,pt.prod_id,pt.trm_id,pt.tp_eval,pt.tmed_verif,pt.tprob,pt.tacciones,pt.prog,pt.eval,pt.activo
                    ...
                    Inner Join _productos as prod On prod.com_id=c.com_id
                    Inner Join _productos_trimestral as pt On prod.prod_id=pt.prod_id
                    where poa.prog=\''.$programa.'\' and pt.testado!=\'3\' and pt.trm_id='.$trimestre.' and pt.tp_eval='.$tipo_eval.'';
        }
    ...
    public function lista_apertura_programas_regional($dep_id,$tp_id){
            if($this->gestion>2022){
                ...
                $sql = '
                select poa.dep_id,apg.aper_programa,apg.aper_proyecto,apg.aper_actividad,apg.aper_descripcion,count(*) total_actividad
                from lista_poa_gastocorriente_nacional('.$this->gestion.') poa
                Inner Join aperturaprogramatica as apg On apg.aper_programa=poa.prog
                Inner Join _componentes as c On c.pfec_id=poa.pfec_id
                Inner Join _productos as prod On prod.com_id=c.com_id
                where poa.dep_id='.$dep_id.' and c.estado!=\'3\' and prod.estado!=\'3\' and apg.aper_asignado=\'1\'
                group by poa.dep_id,apg.aper_programa,apg.aper_proyecto,apg.aper_actividad,apg.aper_descripcion
                order by apg.aper_programa asc';
            }
    ...
    /// nro de actividades (form 4) alineados a los programas hasya el trimestre vigente x REGIONAL
    public function nro_form4_programadas_regional($dep_id,$programa){
        $sql = '
            select poa.dep_id,poa.prog aper_programa,count(pr.prod_id) total
                ...
                where poa.dep_id='.$dep_id.' and poa.prog=\''.$programa.'\' and c.estado!=\'3\' and pr.estado!=\'3\'
    ...
    /*----- NUMERO DE FORM 4 PROGRAMADAS POR TRIMESTRE / APERTURA REGIONAL 2020-2021-2022-2023 ------*/
/*    public function nro_operaciones_programadas_regional($dep_id,$programa,$trimestre,$tp_id){
        ...
        if($this->gestion>2022){
            ...
            $sql = '
            select poa.dep_id,apg.aper_programa,count(*) total
            from lista_poa_gastocorriente_nacional('.$this->gestion.') poa
            Inner Join aperturaprogramatica as apg On apg.aper_programa=poa.prog
            Inner Join _componentes as c On c.pfec_id=poa.pfec_id
            Inner Join _productos as prod On prod.com_id=c.com_id
            ...
            where poa.dep_id='.$dep_id.' and apg.aper_programa=\''.$programa.'\' and c.estado!=\'3\' and prod.estado!=\'3\' and apg.aper_asignado=\'1\'
            group by poa.dep_id,apg.aper_programa';
        }
    ...
    /*--- LISTA ACTIVIDADES (FORM 4) EVALUADAS (CUMPLIDAS-PROCESO-NO CUMPLIDAS) APERTURA REGIONAL 2024---*/
    public function list_operaciones_evaluadas_regional_trimestre($dep_id,$programa,$trimestre,$tipo_eval,$tp_id){
        $sql = 'select poa.dep_id,poa.prog aper_programa,poa.proy aper_proyecto,poa.act aper_actividad,poa.proy_id,c.com_id,pt.prod_id,pt.trm_id,pt.tp_eval,pt.tmed_verif,pt.tprob,pt.tacciones,pt.prog,pt.eval,pt.activo
                    ...
                    Inner Join _productos as prod On prod.com_id=c.com_id
                    Inner Join _productos_trimestral as pt On prod.prod_id=pt.prod_id
                    where poa.dep_id='.$dep_id.' and poa.prog=\''.$programa.'\' and pt.testado!=\'3\' and (pt.trm_id>\'0\' and pt.trm_id<='.$trimestre.') and pt.tp_eval='.$tipo_eval.'';

        $query = $this->db->query($sql);
        return $query->result_array();
    }

    /*--- LISTA OPERACIONES EVALUADAS (CUMPLIDAS-PROCESO-NO CUMPLIDAS) APERTURA REGIONAL 2020-2021-2022-2023---*/
/*    public function list_operaciones_evaluadas_regional_trimestre($dep_id,$programa,$trimestre,$tipo_eval,$tp_id){
        if($this->gestion>2022){
            $sql = 'select poa.dep_id,poa.prog aper_programa,poa.proy aper_proyecto,poa.act aper_actividad,poa.proy_id,c.com_id,pt.prod_id,pt.trm_id,pt.tp_eval,pt.tmed_verif,pt.tprob,pt.tacciones,pt.prog,pt.eval,pt.activo
                    from lista_poa_gastocorriente_nacional('.$this->gestion.') poa 
            ...
            $sql = 'select poa.dep_id,poa.prog aper_programa,poa.proy aper_proyecto,poa.act aper_actividad,poa.proy_id,c.com_id,pt.prod_id,pt.trm_id,pt.tp_eval,pt.tmed_verif,pt.tprob,pt.tacciones,pt.prog,pt.eval,pt.activo
                    ...
                    Inner Join _productos as prod On prod.com_id=c.com_id
                    Inner Join _productos_trimestral as pt On prod.prod_id=pt.prod_id
                    where poa.dep_id='.$dep_id.' and poa.prog=\''.$programa.'\' and pt.testado!=\'3\' and pt.trm_id='.$trimestre.' and pt.tp_eval='.$tipo_eval.'';
        }
...
///// DISTRITAL
    ...
    public function lista_apertura_programas_distrital($dist_id,$tp_id){
            if($this->gestion>2022){
                ...
                Inner Join _componentes as c On c.pfec_id=poa.pfec_id
                Inner Join _productos as prod On prod.com_id=c.com_id
                where poa.dist_id='.$dist_id.' and c.estado!=\'3\' and prod.estado!=\'3\' and apg.aper_asignado=\'1\'
                group by poa.dist_id,apg.aper_programa,apg.aper_proyecto,apg.aper_actividad,apg.aper_descripcion
                order by apg.aper_programa asc';
            }
    ...
    /// nro de actividades (form 4) alineados a los programas hasya el trimestre vigente DISTRITAL
    public function nro_form4_programadas_distrital($dist_id,$programa){
        $sql = '
            select poa.dist_id,poa.prog aper_programa,count(pr.prod_id) total
                ...
                where poa.dist_id='.$dist_id.' and poa.prog=\''.$programa.'\' and c.estado!=\'3\' and pr.estado!=\'3\'
    ...
    /*public function nro_operaciones_programadas_distrital($dist_id,$programa,$trimestre,$tp_id){
        ...
        if($this->gestion>2022){
            ...
            where poa.dist_id='.$dist_id.' and apg.aper_programa=\''.$programa.'\' and c.estado!=\'3\' and prod.estado!=\'3\' and apg.aper_asignado=\'1\'
            group by poa.dist_id,apg.aper_programa';
        }
    ...
    public function list_operaciones_evaluadas_distrital_trimestre($dist_id,$programa,$trimestre,$tipo_eval,$tp_id){
        if($this->gestion>2022){
            ...
            $sql = 'select poa.dist_id,poa.prog aper_programa,poa.proy aper_proyecto,poa.act aper_actividad,poa.proy_id,c.com_id,pt.prod_id,pt.trm_id,pt.tp_eval,pt.tmed_verif,pt.tprob,pt.tacciones,pt.prog,pt.eval,pt.activo
                    ...
                    Inner Join _productos as prod On prod.com_id=c.com_id
                    Inner Join _productos_trimestral as pt On prod.prod_id=pt.prod_id
                    where poa.dist_id='.$dist_id.' and poa.prog=\''.$programa.'\' and pt.testado!=\'3\' and pt.trm_id='.$trimestre.' and pt.tp_eval='.$tipo_eval.'';
        }
