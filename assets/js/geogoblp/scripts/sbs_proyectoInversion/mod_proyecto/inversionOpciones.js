...
function guardarCoordenada(){
    ...
    var v_ubicacion =JSON.parse(coordenada);
                                ...
                                for (var i = 0, len = v_puntos.length; i < len; i++) {
                                   str_puntos='POINT('+procesax(v_puntos[i].lat,v_puntos[i].lng)+' '+procesay(v_puntos[i].lat,v_puntos[i].lng)+')';
                                   geo_geometry+=str_puntos;
                                        if ((len-1)==i) {  
                                            geo_geometry+='';
                                        }
                                        else{
                                ...
                                for (var i = 0, len = v_lineas.length; i < len; i++) {
                                    ...
                                    str_lineas+='))';
                                    //alert(str_lineas)
                                    geo_geometry+=str_lineas;
                                        if ((len-1)==i) { geo_geometry+=''; 
                                        }
                                        else{
                                            geo_geometry+='|'; 
                                        };
                                ...
                                for (var i = 0, len = v_poligonos.length; i < len; i++) {
                                    ...
                                    //cerramos anillo
                                    str_poligon+=' '+procesax(linea[0].lat,linea[0].lng)+' '+procesay(linea[0].lat,linea[0].lng)+''; 
                                    str_poligon+=')))';
                                    //alert(str_poligon)
                                    geo_geometry+=str_poligon;
                                    if ((len-1)==i) {  
                                        }
                                        else{
                                            geo_geometry+='|'; 
    ...
    if($.isNumeric(lat) && $.isNumeric(lng)){
        if(cod_territorio){
            if(pts+lin+cir+pol <= 1){
                ...
                $.ajax({
                    url: 'ubicacionGeoreferencial_graba.php?lat='+lat+'&lng='+lng+'&id_p='+id_p+'&ubicacionExacta='+coordenada+'&cod_territorio='+cod_territorio+'&zoom='+zoom+'&geo='+geo+'&geom='+geo_geometry,
                    type: "GET",
                    success: function( resp ) {
                        if(resp==1){
                            alert('Se grabo la ubicación georeferencial del proyectos');
                            location.reload()
